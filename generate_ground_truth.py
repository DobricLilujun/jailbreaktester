...
def generate_ground_truth(
    json_folder: str, oracle_fct: callable, output: str = "output.json"
):
    """
...
def synchronize_ground_truth(
    ...
    if (
        data_with_gt["data_set_path"] != data_without_gt["data_set_path"]
        or data_with_gt["data_set_type"] != data_without_gt["data_set_type"]
    ):
...
requests = print_results_successful_non_successful(
    "./ground_truth/[GT_2] dataset_JailBreakBench_failed_and_successfull.json"
)
...
print(
    ...
    requests["successful_jailbreaks_count"]
    + requests["non_successful_jailbreaks_count"],
)
