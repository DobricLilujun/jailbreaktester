...
data_set_path = (
    "/home/linux/Developpement/jailbreaktester/dataset/jailbreakBench_full_success.json"
)
...
def run_campaign(
    name, llm_model, data_set_type, data_set_path, classifier, classifier_options
):
    command = [
        "python",
        "main.py",
    ...
    while attempt < MAX_RETRIES:
        try:
            ...
            subprocess.run(
                command, check=True
            )  # check=True pour lever une erreur si le processus échoue
            print(f"Campagne '{name}' terminée avec succès.")
            return  # Si succès, on sort de la fonction
        except subprocess.CalledProcessError as e:
            attempt += 1
            print(
                f"Échec de la campagne '{name}' (tentative {attempt}/{MAX_RETRIES}). Erreur: {e}"
            )
            if attempt < MAX_RETRIES:
                print(
                    f"Nouvelle tentative pour la campagne '{name}' dans {RETRY_DELAY / 60} minutes."
                )
    ...
    for threshold in thresholds:
        for pert_type in dict_pert:
            for copies in num_copies:
                ...
                classifier_options = classifier_options_template.format(
                    threshold=threshold, num_copies=copies, pert_type=pert_type
                )
